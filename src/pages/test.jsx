...
const handleRemoveItem = async (item) => {
 ...
 const handlePlaceOrder = async (paymentMode) => {
  ...
  setSelectedPaymentMethod(paymentMode); // ✅ Save selection
  setIsProcessing(paymentMode); // ✅ Show loading on button

  if (paymentMode === "COD") {
    // ✅ Show confirmation popup for COD
    setPopupConfig({
      type: "warning",
    ...
    setPopupOpen(true);
    setIsProcessing(false); // ✅ Reset processing since we’re waiting for confirmation
    return;
  }

  // ✅ Online payment flow (Stripe, etc.)
  const orderData = {
  ...
  try {
    ...
    if (submitOrder.fulfilled.match(resultAction)) {
      ...
      // If no redirect URL, assume success
      setPopupConfig({
        type: "success",
        title: "Order Placed!",
        subText: "Your order has been successfully placed.",
            ...
            <span className="text-red-500 font-semibold text-xs whitespace-nowrap">
              ...
              <div className="flex justify-between items-center sm:flex-col sm:items-end sm:gap-4 mt-2 sm:mt-0">
                <div className="text-sm sm:text-base font-semibold text-gray-900">
                  {formatPrice(
                    item.price + item.add_ons.reduce((sum, a) => sum + (a?.price || 0), 0)
                  )}
                </div>
